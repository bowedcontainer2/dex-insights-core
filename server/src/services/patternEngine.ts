...
function toLocalHour(utcTimeStr: string): number {
  const utc = new Date(utcTimeStr);
  const local = new Date(utc.toLocaleString('en-US', { timeZone: config.timezone }));
  return local.getHours() + local.getMinutes() / 60;
}

function toLocalTimeStr(utcTimeStr: string): string {
  const utc = new Date(utcTimeStr);
  return new Date(utc.toLocaleString('en-US', { timeZone: config.timezone }))
    .toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });
}

// ── Morning Spike (Dawn Phenomenon) ────────────────────────────────

export function detectMorningSpike(readings: Reading[], date: string): PatternEvent[] {
  const nightReadings = readings.filter((r) => {
    const h = toLocalHour(r.system_time);
    return h >= 3 && h <= 5;
  });
...
export function detectRapidRise(readings: Reading[], date: string): PatternEvent[] {
  ...
  // Sliding window of 6 readings (~30 min at 5-min intervals)
  for (let i = 0; i <= readings.length - 6; i++) {
    const rise = readings[i + 5].value - readings[i].value;
    if (rise >= 40) {
      // Merge if within 15 min of previous event
...
export function detectNocturnalLow(readings: Reading[], date: string): PatternEvent[] {
  const events: PatternEvent[] = [];
  // Filter to midnight–6 AM local time
  const nightReadings = readings.filter((r) => {
    const h = toLocalHour(r.system_time);
    return h >= 0 && h < 6;
  });
...
export function detectHighVariability(readings: Reading[], date: string): PatternEvent[] {
  if (readings.length < 12) return []; // Need at least 1 hour of data
...
export function detectElevatedOvernight(readings: Reading[], date: string): PatternEvent[] {
  const nightReadings = readings.filter((r) => {
    const h = toLocalHour(r.system_time);
    return h >= 0 && h < 6;
  });

  if (nightReadings.length < 6) return []; // Need at least 30 min of overnight data
...
export function detectPostMealCrash(readings: Reading[], date: string): PatternEvent[] {
  ...
  // Look for rise of 30+ mg/dL followed by drop below pre-rise baseline within 3 hours
  for (let i = 0; i < readings.length - 6; i++) {
    const baseline = readings[i].value;
    // Find a peak within the next ~90 min (18 readings)
    let peakIdx = -1;
    ...
    // After the peak, look for a drop below baseline within 3 hours of the start
    const crashWindow = Math.min(i + 36, readings.length); // 36 readings = 3 hours
    let crashIdx = -1;
    let crashVal = peakVal;
    for (let j = peakIdx + 1; j < crashWindow; j++) {
      if (readings[j].value < crashVal) crashVal = readings[j].value;
      if (readings[j].value < baseline - 10) { // Dropped 10+ below where it started
        crashIdx = j;
        break;
      }
    }
    ...
    // Avoid duplicate events — skip ahead if this overlaps a previous one
    const last = events[events.length - 1];
    if (last) {
      const lastEndMs = new Date(last.endTime!).getTime();
      const thisStartMs = new Date(readings[i].system_time).getTime();
...
export function analyzeDay(readings: Reading[], date: string): {
  events: PatternEvent[];
  stats: Omit<DailyStats, 'date'> | null;
} {
  const events = [
    ...detectMorningSpike(readings, date),
    ...
    ...detectNocturnalLow(readings, date),
    ...detectHighVariability(readings, date),
    ...detectElevatedOvernight(readings, date),
